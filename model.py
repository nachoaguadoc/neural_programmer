...
class Graph():
  ...
  def __init__(self, utility, batch_size, max_passes, mode="train"):
    ...
    self.batch_word_column_descriptions = tf.placeholder(tf.int32,[batch_size, self.num_word_cols, self.utility.FLAGS.max_description])
    self.batch_word_column_description_mask = tf.placeholder(tf.float64,[batch_size, self.num_word_cols, self.utility.FLAGS.max_description, self.utility.FLAGS.embedding_dims])
    ...
    self.batch_number_column_descriptions = tf.placeholder(tf.int32,[batch_size, self.num_cols, self.utility.FLAGS.max_description])
    self.batch_number_column_description_mask = tf.placeholder(tf.float64,[batch_size, self.num_cols, self.utility.FLAGS.max_description, self.utility.FLAGS.embedding_dims])
  ...
  def get_steps(self):
    debug = {'ops': self.debug_ops, 'cols': self.debug_cols, 'rows': self.debug_rows, 'soft_ops': self.debug_soft_ops, 'soft_cols': self.debug_soft_cols, 'number_comp': self.debug_number_comp, 'word_match': self.debug_word_match}
    return debug
